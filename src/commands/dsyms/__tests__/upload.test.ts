...
describe('execute', () => {
  ...
  test('should succeed with folder input', async () => {
    const {context, code} = await runCLI('./src/commands/dsyms/__tests__/files/')
    ...
    expect(output[2]).toContain('Will look for dSYMs in src/commands/dsyms/__tests__/files/')
    expect(output[3]).toContain(
      'Uploading dSYM with BD8CE358-D5F3-358B-86DC-CBCF2148097B from src/commands/dsyms/__tests__/files/test.dSYM'
    )
    expect(output[6]).toContain('Handled 1 dSYM with success in')
  })

  test('should succeed with zip file input', async () => {
    const {context, code} = await runCLI('./src/commands/dsyms/__tests__/files/test.zip')
    const outputString = context.stdout.toString()
    const output = outputString.split(EOL)
    ...
    expect(code).toBe(0)
    expect(output[1]).toContain('Starting upload with concurrency 20. ')
    expect(output[2]).toContain('Will look for dSYMs in src/commands/dsyms/__tests__/files/test.zip')
