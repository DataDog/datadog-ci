...
export class LambdaFlareCommand extends Command {
  ...
  public async execute() {
    ...
    try {
      ...
      // Write info file
      const infoFilePath = path.join(rootFolderPath, INFO_FILE_NAME)
      const framework = 'Todo'
      generateInfoFile(infoFilePath, framework, this.isDryRun)
      this.context.stdout.write(`• Saved info file to ${infoFilePath}\n`)
 ...
 * @param infoFilePath path to the info file
 * @param framework the framework used
 * @param isDryRun whether or not this is a dry run
 */
export const generateInfoFile = (infoFilePath: string, framework: string, isDryRun: boolean) => {
  // TODO write tests, get framework
  const lines: string[] = []
  ...
  writeFile(infoFilePath, lines.join('\n'))
}
