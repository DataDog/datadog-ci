...
describe('lambda flare', () => {
  ...
  describe('getFramework', () => {
    ...
    it('returns undefined when no framework files exist', () => {
      ;(fs.readdirSync as jest.Mock).mockReturnValue(['abc.md', 'Dockerfile', 'test.js', 'README.md'])
      expect(getFramework()).toBeUndefined()
    })
  })
  ...
  describe('generateInsightsFile', () => {
    ...
    it('should call writeFile with correct content when isDryRun is false', () => {
      generateInsightsFile(insightsFilePath, false)

      expect(writeFileSpy).toHaveBeenCalledTimes(1)

      const receivedContent = writeFileSpy.mock.calls[0][1]
      expect(receivedContent).toContain('_Autogenerated file from `lambda flare`_')
      expect(receivedContent).not.toContain('</br>_This command was run in dry mode._')
      expect(receivedContent).toContain('**Run Location**: `mock-folder`')
      expect(receivedContent).toContain('**CLI Version**: ')
      expect(receivedContent).toContain('**Framework**: `Unknown`')
      expect(receivedContent).toContain('**Timestamp**: `')
    })

    it('should call writeFile with correct content when isDryRun is true', () => {
      generateInsightsFile(insightsFilePath, true)

      expect(writeFileSpy).toHaveBeenCalledTimes(1)

      const receivedContent = writeFileSpy.mock.calls[0][1]
      expect(receivedContent).toContain('</br>_This command was run in dry mode._')
    })
  })
