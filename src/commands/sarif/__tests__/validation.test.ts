...
describe('validation', () => {
  test('should find no error for SARIF file', () => {
    const err = checkForError('./src/commands/sarif/__tests__/fixtures/valid-results.sarif')
    expect(err.length).toBe(0)
  })
  test('results must have rules', () => {
    const err = checkForError('./src/commands/sarif/__tests__/fixtures/test_validation/invalid-sarif-no-rule.sarif')
