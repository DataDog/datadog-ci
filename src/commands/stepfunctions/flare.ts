...
export class StepFunctionsFlareCommand extends Command {
  ...
  private generateInsightsFile(
    ...
    // Configuration Analysis - FIRST SECTION
    ...
    // Check log level
    if (!config.loggingConfiguration || config.loggingConfiguration.level !== 'ALL') {
      issues.push('**Log level must be set to "ALL"** for complete observability')
    }

    // Check includeExecutionData
    if (!config.loggingConfiguration || !config.loggingConfiguration.includeExecutionData) {
      issues.push('**Include Execution Data must be enabled** to capture input/output in logs')
    }

    // Check X-Ray tracing
    if (config.tracingConfiguration?.enabled) {
      warnings.push('**X-Ray tracing is enabled** - This is duplicative with Datadog tracing and will increase costs')
    }
    ...
    // Check Datadog tags
    const ddTraceEnabled = tags['DD_TRACE_ENABLED'] || tags['dd_trace_enabled']
    const ddTraceSampleRate = tags['DD_TRACE_SAMPLE_RATE'] || tags['dd_trace_sample_rate']

    if (!ddTraceEnabled || (ddTraceEnabled.toLowerCase() !== 'true' && ddTraceEnabled !== '1')) {
      issues.push('**DD_TRACE_ENABLED must be set to true** on either the Step Function tags or the Datadog forwarder')
    }
    ...
    if (issues.length === 0 && warnings.length === 0) {
      lines.push('\n**Configuration looks good!** All requirements are met.')
    }
    ...
    // Tags Section - As a subsection of State Machine Configuration
    lines.push('\n### Tags')
    ...
    if (tagKeys.length > 0) {
      ...
      lines.push('No tags found on this state machine.')
    }
