...
export class DeploymentGateCommand extends Command {
  ...
  public async execute() {
    ...
    if (!this.config.appKey) {
      this.logger.error(
        `Neither ${chalk.red.bold('DATADOG_APP_KEY')} nor ${chalk.red.bold('DD_APP_KEY')} is in your environment.`
      )
    ...
    this.logger.info(`Timeout: ${timeoutSeconds} seconds, Fail on error: ${this.failOnError}`)
  ...
  private getApiHelper(): APIHelper {
    ...
    return apiConstructor(baseAPIURL, this.config.apiKey!, this.config.appKey!)
  }
