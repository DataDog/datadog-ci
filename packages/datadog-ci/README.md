...
> [!NOTE]  
> To check the version of the standalone binary without running it, run `strings path/to/datadog-ci | grep STANDALONE_BINARY_VERSION` or `grep --text STANDALONE_BINARY_VERSION path/to/datadog-ci`.
> This is useful to determine the version when unable to run `datadog-ci --version`, for example because of segmentation faults or architecture mismatch.
