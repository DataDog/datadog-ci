...
/**
 * What `instrument` and `uninstrument` have in common: which task definitions to rewrite, which
 * services to roll the new revision out to, and how to reach AWS. Both name their targets the same
 * way, so the flags naming them are declared and validated once.
 *
 * A subcommand adds its own flags, reports them through `additionalConfig`, and validates them in
 * `additionalErrors`.
 */
export abstract class EcsFargateCommand extends BaseCommand {
  ...
  private taskDefinitions = Option.Array('--task-definition,--taskDefinition', {
    description: 'The family, family:revision, or ARN of the task definition. Can be specified multiple times.',
  })
  ...
  private ecsServices = Option.Array('--ecs-service,--ecsService', {
    description:
      'The name of an ECS service to update to the newly registered revision, so that the change rolls out without a manual deployment. Can be specified multiple times.',
  })
  ...
  /**
   * The configuration the subcommand's own flags contribute.
   */
  ...
  /**
   * The configuration to run with, along with everything wrong with it. The caller reports the
   * problems, so that a run with several mistakes in it names them all at once.
   */
  public async ensureConfig(): Promise<[EcsFargateConfigOptions, string[]]> {
    ...
    if (!config.taskDefinitions?.length) {
      errors.push('No task definitions specified. Use --task-definition.')
    }
    ...
    if (duplicates.length > 0) {
      errors.push(
        `--task-definition names the same task definition family more than once (${duplicates.join(
          ', '
        )}). A run acts on one revision per family, so name the revision to act on once.`
      )
    }
    ...
    errors.push(...this.additionalErrors(config))
  ...
  /**
   * Everything wrong with the configuration that only the subcommand can tell is wrong.
   */
  protected additionalErrors(_config: EcsFargateConfigOptions): string[] {
    return []
  }
}
