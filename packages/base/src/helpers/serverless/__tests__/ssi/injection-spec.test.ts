...
describe('language injection specifications', () => {
  ...
  test.each([
    ...
    expect(getLanguageCompatibilityErrors({language, libc: 'glibc', version: baseline}, {probeServer: true})).toEqual([
      expect.stringContaining(`later than ${baseline}`),
    ])
  })

  // Only Cloud Run keeps the tracer container alive to serve a readiness probe. Platforms that copy
  // and exit place no floor on the tracer version, so they get none by default.
  test.each([
    ['java', '1.65.1'],
    ['nodejs', '6.10.0'],
    ['csharp', '3.51.1'],
    ...
    ['ruby', '2.41.0'],
    ['php', '1.23.3'],
  ] as const)('accepts the %s probe-server baseline without a probe server', (language, baseline) => {
  ...
  test.each([
    ...
    expect(getLanguageCompatibilityErrors({language: 'python', libc: 'glibc', version}, {probeServer: true})).toEqual(
      expected
    )
  })
