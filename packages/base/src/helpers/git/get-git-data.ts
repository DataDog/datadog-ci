...
export const gitRemote = async (git: GitClient): Promise<string> => {
...
export const getDefaultRemoteName = async (git: GitClient): Promise<string> => {
  try {
...
export const gitHash = async (git: GitClient): Promise<string> => git.revparse('HEAD')

// Returns the tracked files of the current repository.
export const gitTrackedFiles = async (git: GitClient): Promise<string[]> => {
  const files = await git.raw('ls-files')

  return files.split(/\r\n|\r|\n/)
}

export const gitBranch = async (git: GitClient): Promise<BranchSummary> => git.branch()

export const gitCurrentBranch = async (git: GitClient): Promise<string> => {
  const branch = await git.raw(['branch', '--show-current'])

  return branch.trim()
}

export const gitMessage = async (git: GitClient): Promise<string> => git.show(['-s', '--format=%s'])

// Returns the author and committer information of the current commit.
export const gitAuthorAndCommitter = async (git: GitClient): Promise<GitAuthorAndCommitterMetadata> => {
...
export const gitRepositoryURL = async (git: GitClient): Promise<string> =>
  git.listRemote(['--get-url']).then((url) => url.trim())

export const gitLocalCommitShas = async (git: GitClient, branchName: string): Promise<string[]> => {
