...
import {enableCloudwatchLogs, getRoleName} from '../functions/cloudwatch'
import {
...
export class PluginCommand extends LambdaEnableCloudwatchCommand {
  private config: any = {
    functions: [],
  ...
  public async execute(): Promise<0 | 1> {
    ...
    this.context.stdout.write(cloudwatchRenderer.renderCloudwatchHeader('enable', this.dryRun))
  ...
  private async processRegion(
    ...
    if (this.dryRun) {
      for (const fn of functionARNs) {
        try {
          ...
          this.context.stdout.write(cloudwatchRenderer.renderDryRunFunctionAction('enable', fn, roleName))
        } catch (err) {
    ...
    await Promise.all(
      functionARNs.map(async (fn) => {
        try {
          const roleName = await getRoleName(lambdaClient, fn)
          await enableCloudwatchLogs(iamClient, roleName)
          this.context.stdout.write(cloudwatchRenderer.renderFunctionSuccess('enable', fn, roleName))
        } catch (err) {
          hasError = true
          this.context.stdout.write(cloudwatchRenderer.renderFunctionError(fn, err))
        }
